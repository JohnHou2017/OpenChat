...
function insertDocument(db, doc, targetCollection) {
    var retid = 0;
    while (1) {
        var cursor = db.collection(targetCollection).find({}, { _id: 1 }).sort({ _id: -1 }).limit(1);
        var seq = cursor.hasNext() ? cursor.next()._id + 1 : 1;
        exports.viewLogger.error("bbbnewid : " + seq);
        if (isNaN(seq))
            seq = 1;
        doc._id = seq;
        exports.viewLogger.error("newid : " + seq);
        exports.viewLogger.error("targetCollection : " + targetCollection);
        var results = db.collection(targetCollection).insert(doc);
        if (results) {
            exports.viewLogger.error("successnewid : " + seq);
        }
        exports.viewLogger.error(results.writeError);
        //viewLogger.error(results.writeError.code);
        exports.viewLogger.error(results.nInserted);
